...
// failingPurge refuses to hard-delete the rows it was told about, the way a
...
func (f *failingPurge) HardDeleteNode(ctx context.Context, id int64) error {
	if f.fail[id] {
...
func TestPurgeExpired_TriesARowThatFailsOnce(t *testing.T) {
	conn, store, sid := sweepFixture(t)
	stuck := &failingPurge{Store: store, fail: map[int64]bool{}}
	for i := 0; i < fullBatch; i++ {
