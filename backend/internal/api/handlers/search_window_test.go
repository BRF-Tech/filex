...
func TestManagerSearch_FallbackRanksBeforeItCuts(t *testing.T) {
	...
	// ⚠ What "cut" means: the database was asked for its most selective word
	// (`report` here — the longest) and that window was full, so rows past it
	// were never read. One row comes back, and the answer still says it may
	// not be all of them, because the server cannot know that it is.
	...
	assert.True(t, *narrow.Truncated, "a full window is a cut window, however few of its rows survive")
}
