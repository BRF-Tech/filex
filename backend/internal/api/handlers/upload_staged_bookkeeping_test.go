...
// deleteRefusingStore never lets a session row be deleted — the database that
// is busy for exactly as long as the transfer's cleanup runs.
type deleteRefusingStore struct {
	db.Store
}

func (s *deleteRefusingStore) DeleteStagedUpload(ctx context.Context, id string) error {
	return errors.New("database is locked (simulated)")
}

// ⚠ The sweeper skips `committing` rows on purpose, and the quota reservation
// counts them. A row whose delete failed after the bytes were stored would
// therefore sit there for ever, holding its user's quota, with no staging left
// to justify it. The transfer demotes it so the sweeper can finish the job.
func TestStagedTransfer_ARowItCannotDeleteIsHandedToTheSweeper(t *testing.T) {
	f := newStagedFixtureWith(t, func(d *api.Deps) {
		d.Store = &deleteRefusingStore{Store: d.Store}
	})
	body := randomBytes(10_000)

	sessionID, nodeID, opID := f.stageAndCommitOnce(t, "fatura.bin", body)
	require.Equal(t, "ok", f.waitForOp(t, opID))
	assert.Equal(t, model.TransferStateStored, f.nodeState(t, nodeID))

	sess, err := f.store.GetStagedUpload(context.Background(), sessionID)
	require.NoError(t, err, "the row could not be deleted, so it is still there")
	assert.NotEqual(t, model.StagedUploadCommitting, sess.State,
		"a stranded committing row is never swept and never releases its quota")

	f.deps.StagedUploads.TTL = 0
	rows, _ := f.deps.StagedUploads.Sweep(context.Background())
	assert.Equal(t, 0, rows, "the delete still refuses, but the sweeper may now try")
	sess, err = f.store.GetStagedUpload(context.Background(), sessionID)
	require.NoError(t, err)
	assert.Equal(t, model.StagedUploadFailed, sess.State)
}
