...
func slowTrash(t *testing.T, n int) (*httptest.Server, db.Store, *heldPurge, int64) {
	...
	svc := trash.New(held, nil, nil)
	queue := ops.New(conn, nil)
	require.NoError(t, queue.Migrate(context.Background()))
	queue.SetTrashEmptier(svc)
...
// The run is an ops row of the tenant that asked for it: its admin finds it in
// the operations list and cancels it there, the way every queued operation is
// cancelled; another tenant's admin neither lists, reads nor cancels it — and
// its trash is untouched.
func TestTrashEmpty_IsAnOpsRowOfItsTenant(t *testing.T) {
	var held *heldPurge
	srv, client, store := testutil.NewTestServerWith(t,
		func(c *config.Config) { c.MultiTenant = true },
		func(d *api.Deps) {
			held = &heldPurge{Store: d.Store, reached: make(chan struct{}), release: make(chan struct{})}
			d.Trash = trash.New(held, d.StorageResolver, d.Quota)
	...
	status, body := doJSON(t, client, http.MethodPost, srv.URL+"/api/admin/trash/empty", map[string]any{})
	require.Equal(t, http.StatusAccepted, status, "%v", body)
	opID := int64(body["op_id"].(float64))
	require.NotZero(t, opID, "the answer names its ops row")
	waitClosed(t, held.reached, "the purge to be under way")
	opURL := fmt.Sprintf("%s/api/files/ops/%d", srv.URL, opID)

	listed := func(c *http.Client) bool {
		status, body := doJSON(t, c, http.MethodGet, srv.URL+"/api/files/ops", nil)
		require.Equal(t, http.StatusOK, status, "%v", body)
		rows, _ := body["ops"].([]any)
		for _, r := range rows {
			row := r.(map[string]any)
			if int64(row["id"].(float64)) == opID {
				assert.Equal(t, "trash-empty", row["kind"])
				return true
			}
		}
		return false
	}
	assert.True(t, listed(client), "the tenant's admin sees its run among the operations")

	jar, err := cookiejar.New(nil)
	require.NoError(t, err)
	stranger := &http.Client{Jar: jar}
	testutil.LoginAs(t, srv, stranger, theirs.adminEmail, theirs.adminPass)
	assert.False(t, listed(stranger), "another tenant's admin does not")
	status, _ = doJSON(t, stranger, http.MethodGet, opURL, nil)
	assert.Equal(t, http.StatusNotFound, status, "nor reads it")
	status, _ = doJSON(t, stranger, http.MethodPost, opURL+"/cancel", nil)
	assert.Equal(t, http.StatusNotFound, status, "nor cancels it")

	status, body = doJSON(t, client, http.MethodPost, opURL+"/cancel", nil)
	require.Equal(t, http.StatusOK, status, "%v", body)
	held.free()
	done := untilEmptied(t, client, srv.URL)
	assert.Equal(t, true, done["cancelled"], "%v", done)

	n, err := store.GetNode(t.Context(), theirs.trashed)
	require.NoError(t, err, "another tenant's trashed file was purged")
