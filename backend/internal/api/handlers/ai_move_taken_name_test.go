...
//
// ⚠ An agent's move DE-COLLIDES (the owner's decision for v0.43.0): the item
// lands on a free name beside the taken one and the answer names where it
// landed. The explorer's rename, where a person typed the name, answers 409
// instead (manager_rename_taken_name_test.go). Neither ever replaces a file.
...
func TestAI_Move_OntoATakenName_LandsBesideIt(t *testing.T) {
	...
	assert.Equal(t, "was-here", readFile(t, filepath.Join(hotDir, "b.txt")), "the file that had the name was replaced")
	entry, _ := body["entry"].(map[string]any)
	require.NotNil(t, entry, body)
	landed, _ := entry["path"].(string)
	assert.NotEqual(t, "hot://b.txt", landed, "the answer must name where the item really landed")
	assert.Equal(t, "incoming", readFile(t, filepath.Join(hotDir, filepath.FromSlash(landed[len("hot://"):]))))
	_, err := os.Stat(filepath.Join(hotDir, "a.txt"))
	assert.True(t, os.IsNotExist(err), "a move leaves nothing at the source")
}

func TestAI_Move_AcrossStorages_OntoATakenName_LandsBesideIt(t *testing.T) {
	...
	require.NotNil(t, entry, body)
	landed, _ := entry["path"].(string)
	require.NotEqual(t, "cold://rapor.txt", landed)
	assert.Equal(t, "new", readFile(t, filepath.Join(coldDir, filepath.FromSlash(landed[len("cold://"):]))))
	_, err := os.Stat(filepath.Join(hotDir, "rapor.txt"))
	assert.True(t, os.IsNotExist(err), "the source goes once the copy is verified")
}
