...
//
// ⚠ A change inside filex's own directories is not recorded — the same rule
// the hub applies to what it announces (internalChange): a version snapshot,
// a trash move's bookkeeping or the desktop's open-with working copy is
// nothing a sync client lists, and recording it answered "changed" to every
// client paired at the storage root, each of which then walked its tree for
// nothing. Only when EVERY name the event carries is filex's own: a user file
// renamed onto such a name still reads as changed (a false "changed" costs one
// walk, a false "unchanged" a missed edit).
...
// ownChange reports a change that touched nothing but filex's own
// directories (see EmitChange).
func ownChange(dir string, ev ChangeEvent) bool {
	if syspath.Hidden(dir) {
		return true
	}
	if ev.Name == "" && ev.NewName == "" {
		return false
	}
	own := func(name string) bool { return name == "" || syspath.IsName(name) }
	return own(ev.Name) && own(ev.NewName)
}
