...
// filex's own directories are nothing a sync client lists. A version snapshot,
// the trash's bookkeeping or the desktop's open-with working copy used to make
// every client paired at the storage root walk its whole tree for nothing.
func TestChangeLog_FilexsOwnDirectoriesAreNotAChange(t *testing.T) {
	l := NewChangeLog(16)
	_, cur := l.ChangedSince(1, "", "")
	l.EmitChange(1, ".versions/42", ChangeEvent{Action: "upload", Name: "3"})
	l.EmitChange(1, ".filex-open", ChangeEvent{Action: "upload", Name: "a1b2c3-teklif.docx"})
	l.EmitChange(1, "", ChangeEvent{Action: "mkdir", Name: ".filex-open"})
	l.EmitChange(1, "docs/.thumbs", ChangeEvent{Action: "upload", Name: "k.jpg"})
	if changed, _ := l.ChangedSince(1, "", cur); changed {
		t.Fatal("a change inside filex's own directories read as a change of the storage")
	}
	// A person's file renamed onto such a name is still a change: the file is
	// gone from the listing.
	l.EmitChange(1, "docs", ChangeEvent{Action: "rename", Name: "rapor.txt", NewName: ".versions"})
	if changed, _ := l.ChangedSince(1, "docs", cur); !changed {
