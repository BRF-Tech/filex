	...
	for _, e := range engines() {
		t.Run(e.name, func(t *testing.T) {
			...
			list := func(muted []string, f model.BroadcastFilter, onlyUnread bool, limit, offset int) ([]string, int64) {
				t.Helper()
				rows, total, err := store.ListNotifications(ctx, &reader.ID, onlyUnread, muted, nil, f, limit, offset)
				require.NoError(t, err)
			...
			rows, _, err := store.ListNotifications(ctx, nil, false, nil, nil, only, 50, 0)
			require.NoError(t, err)
			...
			for _, c := range []struct {
				...
				n, err := store.UnreadNotificationCount(ctx, &reader.ID, c.muted, nil, c.f)
				require.NoError(t, err)
				require.EqualValues(t, c.want, n, "%+v muted=%v", c.f, c.muted)
			}

			// The two halves of a per-user read, apart (notify.Service.ListVisible
			// counts the own rows in SQL and walks only the broadcasts). The
			// PostgreSQL clause drops a placeholder when a half goes, so the
			// mute list, the hidden bodies and LIMIT/OFFSET after it must still
			// number correctly.
			own, broadcasts := only, only
			own.OwnOnly = true
			broadcasts.BroadcastsOnly = true
			got, total = list(nil, own, false, 50, 0)
			require.Equal(t, []string{"own"}, got, "OwnOnly")
			require.EqualValues(t, 1, total)
			got, total = list(nil, broadcasts, false, 50, 0)
			require.Equal(t, []string{"alert"}, got, "BroadcastsOnly")
			require.EqualValues(t, 1, total)
			rows, total, err = store.ListNotifications(ctx, &reader.ID, true, []string{"replica_status_report"},
				[]string{"nothing-%"}, model.BroadcastFilter{Except: except.Except, BroadcastsOnly: true, ReaderID: reader.ID}, 1, 0)
			require.NoError(t, err)
			require.Equal(t, []string{"alert"}, bodies(rows), "every clause after a half-read clause, numbered")
			require.EqualValues(t, 1, total)
			for _, c := range []struct {
				f    model.BroadcastFilter
				want int64
			}{{own, 1}, {broadcasts, 1}} {
				n, err := store.UnreadNotificationCount(ctx, &reader.ID, []string{"quota_full"}, []string{"nothing-%"}, c.f)
				require.NoError(t, err)
				require.EqualValues(t, c.want, n, "%+v", c.f)
			}
		})
	}
}
...
// broadcasts (migration 00056) on every engine: a reader's marks are theirs
	...
	for _, e := range engines() {
		t.Run(e.name, func(t *testing.T) {
			...
			unread := func(reader int64) []string {
				t.Helper()
				rows, total, err := store.ListNotifications(ctx, &reader, true, nil, nil, model.BroadcastFilter{ReaderID: reader}, 50, 0)
				require.NoError(t, err)
				out := make([]string, 0, len(rows))
				for _, n := range rows {
				...
				n, err := store.UnreadNotificationCount(ctx, &reader, nil, nil, model.BroadcastFilter{ReaderID: reader})
				require.NoError(t, err)
			...
			readAt := func(reader int64, body string) bool {
				t.Helper()
				rows, _, err := store.ListNotifications(ctx, &reader, false, nil, nil, model.BroadcastFilter{ReaderID: reader}, 50, 0)
				require.NoError(t, err)
				for _, n := range rows {
					if n.Body == body {
						return n.ReadAt != nil
			...
			rows, total, err := store.ListNotifications(ctx, &r2.ID, true, []string{"replica_status_report"}, nil,
			...
			hist := func(reader int64) []string {
				t.Helper()
				rows, _, err := store.ListNotifications(ctx, nil, true, nil, nil, model.BroadcastFilter{ReaderID: reader}, 50, 0)
				require.NoError(t, err)
				out := make([]string, 0, len(rows))
				for _, n := range rows {
					out = append(out, n.Body)
