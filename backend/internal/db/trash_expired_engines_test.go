...
func TestListTrashedExpired_NarrowsToStoragesOnEveryEngine(t *testing.T) {
	for _, e := range engines() {
		t.Run(e.name, func(t *testing.T) {
			...
			all, err := store.ListTrashedExpired(ctx, cutoff, nil, 500)
			require.NoError(t, err)
			assert.ElementsMatch(t, []int64{na, nb}, ids(all), "nil means every storage")

			onlyA, err := store.ListTrashedExpired(ctx, cutoff, []int64{a}, 500)
			require.NoError(t, err)
			assert.Equal(t, []int64{na}, ids(onlyA))

			both, err := store.ListTrashedExpired(ctx, cutoff, []int64{a, b}, 500)
			require.NoError(t, err)
			assert.ElementsMatch(t, []int64{na, nb}, ids(both))

			none, err := store.ListTrashedExpired(ctx, cutoff, []int64{}, 500)
			require.NoError(t, err)
