...
func TestSyncNeverCataloguesFilexsOwnTrees_DirectoryWalk(t *testing.T) {
	...
	// ⚠ At ANY depth (syspath.Sealed): since v0.43 a folder of that name is
	// filex's wherever it sits — hidden from every listing, refused to every
	// write — so the scan does not catalogue one deeper down either. The
	// desktop's open-with working area IS catalogued: the desktop lists it.
