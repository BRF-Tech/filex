...
func TestScopePath(t *testing.T) {
	for raw, want := range map[string]string{
		...
		"docs/.versions/notes": "/docs/.versions/notes", // a user's folder, not filex's tree
	} {
		got, err := filexsync.ScopePath(raw)
		require.NoError(t, err, "ScopePath(%q)", raw)
		assert.Equal(t, want, got, "ScopePath(%q)", raw)
	}
	for _, raw := range []string{"..", "../etc", "docs/../../x", "docs/..", ".versions", "/.versions/42", ".filex-trash/x", ".thumbs", "a\x00b"} {
