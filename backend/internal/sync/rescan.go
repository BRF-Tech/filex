...
// trees (versioning.IsInternalTree), which the walk never catalogues.
func ScopePath(raw string) (string, error) {
	...
	if versioning.IsInternalTree(clean) {
...
func (s *storageSyncer) rescanFolder(ctx context.Context, dir string) (FolderRescan, error) {
	...
	c := &walkCounts{}
	seen, err := s.walk(ctx, dir, &folder.ID, c, list)
	...
	case !guardOK(seen, int(baseline)):
		res.RemovalSkipped = fmt.Sprintf("the listing saw %d of the %d entries the catalogue holds below this folder, under 70%%; nothing was removed", seen, baseline)
	default:
