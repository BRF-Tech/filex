...
ALTER TABLE api_tokens ADD COLUMN usernames VARCHAR(1024) NOT NULL DEFAULT '';

-- Which token username created a share — surfaced in the admin Shares list
-- ("admin (work)"). Empty for session-created shares.
ALTER TABLE shares ADD COLUMN created_via VARCHAR(64) NOT NULL DEFAULT '';

-- +goose Down
ALTER TABLE api_tokens DROP COLUMN usernames;
ALTER TABLE shares DROP COLUMN created_via;
