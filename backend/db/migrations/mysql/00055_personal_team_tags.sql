...
--
-- ⚠ MySQL: `name` and `name_key` are utf8mb4_0900_bin. The table default
-- (utf8mb4_0900_ai_ci) is case- AND accent-insensitive, so under it the
-- UNIQUE key would merge "müşteri" with "musteri" and "şık" with "sık" —
-- different words (see 00041 for the same trap on file names). The fold that
-- IS wanted (case, Turkish i) is done in Go, into name_key; the column itself
-- must compare it byte for byte. The derived columns below carry the same
-- COLLATE, or GROUP BY would merge them the ai_ci way and the join would fail
...
CREATE TABLE IF NOT EXISTS tags (
    ...
    name_key   VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_bin NOT NULL,
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    UNIQUE KEY uq_tags_owner_key (owner_id, name_key),
    UNIQUE KEY uq_tags_tenant_key (tenant_id, name_key),
...
CREATE TABLE IF NOT EXISTS node_tags (
    ...
    KEY idx_node_tags_node (node_id),
    CONSTRAINT fk_node_tags_tag FOREIGN KEY (tag_id) REFERENCES tags(id) ON DELETE CASCADE,
    CONSTRAINT fk_node_tags_node FOREIGN KEY (node_id) REFERENCES nodes(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
FROM (
    SELECT COALESCE(ps.provider_id, 0)                                          AS tenant_id,
           SUBSTR(m.meta_key, 5) COLLATE utf8mb4_0900_bin                       AS name,
           REPLACE(SUBSTR(m.meta_key, 5), 'ı', 'i') COLLATE utf8mb4_0900_bin    AS name_key
    FROM node_meta m
    JOIN nodes n ON n.id = m.node_id
    LEFT JOIN provider_storages ps ON ps.storage_id = n.storage_id
    WHERE m.meta_key LIKE 'tag:%' AND CHAR_LENGTH(m.meta_key) > 4
) x
...
JOIN tags t ON t.kind = 'team'
           ...
           AND t.name_key = REPLACE(SUBSTR(m.meta_key, 5), 'ı', 'i') COLLATE utf8mb4_0900_bin
WHERE m.meta_key LIKE 'tag:%' AND CHAR_LENGTH(m.meta_key) > 4;

DELETE FROM node_meta WHERE meta_key LIKE 'tag:%';

-- +goose Down
INSERT IGNORE INTO node_meta (node_id, meta_key, value)
SELECT DISTINCT nt.node_id, CONCAT('tag:', t.name_key), '1'
FROM node_tags nt
