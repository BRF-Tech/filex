...
-- +goose Down
INSERT OR IGNORE INTO node_meta (node_id, meta_key, value)
SELECT DISTINCT nt.node_id, 'tag:' || t.name_key, '1'
FROM node_tags nt
JOIN tags t ON t.id = nt.tag_id
WHERE t.kind = 'team';
DROP INDEX IF EXISTS idx_node_tags_node;
DROP TABLE IF EXISTS node_tags;
DROP INDEX IF EXISTS uq_tags_tenant_key;
DROP INDEX IF EXISTS uq_tags_owner_key;
