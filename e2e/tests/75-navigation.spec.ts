 ...
 *
 * Guards the bug Burak hit on fm.brf.sh: clicking 'Kök' inside
...
test.describe('Navigation — root crumb + go-up', () => {
  ...
  test('drill in + click root-crumb pops back to storage root', async ({ page, request }) => {
    ...
    // which always has the example/ folder Burak uploaded — that's
    // where the bug happened and that's where the regression matters.
    test.skip(
      !process.env.E2E_FM_HOST?.includes('fm.brf.sh'),
      'requires the fm.brf.sh deployment + s3-test storage with example/ folder',
    );
    ...
    // Drill into example/ (always present on fm.brf.sh).
