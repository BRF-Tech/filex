...
|---|---|
| [`compose/`](compose/) | Docker Compose — `docker-compose.minimal.yml` (one container, SQLite) and `docker-compose.full.yml` (Postgres + Redis + Caddy + opt-in ONLYOFFICE / draw.io / the legacy Convert side-car / MinIO / ClamAV), plus a multi-tenant variant. |
