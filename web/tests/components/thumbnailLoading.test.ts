 ...
 *  count how often each file's thumbnail is asked for. */
async function folder(view: typeof GridView | typeof GalleryView | typeof ListView, count: number, immediate: boolean) {
  ...
  const w = mount(view, { props: { files, selected: new Set<string>(), locale: 'en', thumbSrc } });
  await flushPromises();
  await nextTick();
  return { w, asked, total: () => [...asked.values()].reduce((s, n) => s + n, 0) };
}
