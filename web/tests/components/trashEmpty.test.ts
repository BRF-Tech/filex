...
describe('Trash — empty trash', () => {
  ...
  it('a run can be stopped from its strip, and the stop is said', async () => {
    ...
    const w = await mountTrash();
    postReply = async () => ({
      data: { ok: true, op_id: 41, running: true, total: 500, scanned: 120, purged: 120, failed: 0, bytes: 0 },
    });
    statuses = [
    ...
    await poll();
    expect(w.find('[data-testid="trash-emptying"]').exists()).toBe(false);
    expect(toasts()).toContain('Stopped after 121 items; the rest is still in the trash.');
    expect(toasts().some((m) => /purged/.test(m)), 'a stopped run is not announced as purged').toBe(false);
  });

  it('a run waiting for another purge says so', async () => {
    vi.useFakeTimers();
    listed = [entry(1)];
    statuses = [{ ok: true, op_id: 9, running: true, queued: true, total: 3, scanned: 0, purged: 0, failed: 0 }];
    const w = await mountTrash();
