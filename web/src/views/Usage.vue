...
async function load() {
  loading.value = true;
  try {
    const r = await UsageApi.report(range.value);
    report.value = r;
  ...
  <div class="space-y-4 max-w-4xl">
    ...
    <template v-else>
      ...
      <div class="card card-body space-y-3" data-testid="usage-config">
        ...
        <Select v-model="form.provider" :label="t('usage.fields.provider')">
          <option value="">{{ t('usage.fields.providerNone') }}</option>
        ...
        <div class="flex items-center justify-between gap-2 pt-1">
          <Select v-model.number="range" :label="t('usage.fields.range')" class="max-w-[12rem]">
            <option :value="7">{{ t('usage.ranges.d7') }}</option>
            <option :value="30">{{ t('usage.ranges.d30') }}</option>
            <option :value="90">{{ t('usage.ranges.d90') }}</option>
          </Select>
