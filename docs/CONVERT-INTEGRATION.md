...
```
filex frontend (Vue)
  ConvertModal.vue  ──hidden iframe──►  fm.brf.sh/convert/?embed=1   (convert fork)
...
### convert fork → fm.brf.sh/convert  (subpath, both demo + prod vhosts)
```
# on main (Hetzner):
...
```
Caddy (`/root/caddy/sites.caddy`, container `brf-caddy`) — in BOTH `fm.brf.sh` and
`demo-fm.brf.sh` vhosts:
```
@conv path /convert /convert/*
handle @conv { reverse_proxy 127.0.0.1:8080 }
handle { reverse_proxy 127.0.0.1:5213 }   # 5212 for demo
```
`docker exec brf-caddy caddy reload --config /etc/caddy/Caddyfile`. (⚠ `handle` takes a
...
# demo  /root/filex (5212, demo-fm.brf.sh):  image bump + FILEX_CONVERT_URL=https://demo-fm.brf.sh/convert
# prod  /root/filex-standalone (5213, fm.brf.sh): image bump + FILEX_CONVERT_URL=https://fm.brf.sh/convert
