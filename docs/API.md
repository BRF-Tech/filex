...
| `GET /api/files/capabilities` → `queued` | the changes this server runs as jobs of its operations queue when asked with `queued=1`: `rename` (above) and `restore` (`POST /api/files/manager/restore?queued=1` with `{ node_ids }`, see [TRASH-VERSIONING.md](TRASH-VERSIONING.md#trash-endpoints)). Absent on an older server, which changes inside the request: ask it the old way |
