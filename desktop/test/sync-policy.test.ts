...
test('the folder line: pause and sign-out come first', () => {
  const st = running({ errors: { 'pair-1': 'boom' }, lastError: 'boom' });
  assert.deepEqual(pairView({ pairId: 'pair-1', paused: true, signedOut: true, status: st, minuteOfDay: NOON }), { kind: 'paused' });
  assert.deepEqual(pairView({ pairId: 'pair-1', paused: false, signedOut: true, status: st, minuteOfDay: NOON }), { kind: 'signed-out' });
  assert.deepEqual(pairView({ pairId: 'pair-1', paused: false, signedOut: false, status: null, minuteOfDay: NOON }), { kind: 'starting' });
});
...
test('the folder line: the pair being worked on says what is happening; the others are watching', () => {
  ...
  assert.deepEqual(pairView({ pairId: 'pair-2', paused: false, signedOut: false, status: st, minuteOfDay: NOON }), {
    kind: 'active', phase: 'transfer', done: 3, total: 9,
  });
  assert.deepEqual(pairView({ pairId: 'pair-1', paused: false, signedOut: false, status: st, minuteOfDay: NOON }), { kind: 'watching' });
});

test('the folder line: an error is shown on ITS folder; the engine\'s own on every folder', () => {
  const st = running({ errors: { 'pair-1': 'pair-1: list docs://a: HTTP 502' }, lastError: 'pair-1: list docs://a: HTTP 502' });
  assert.deepEqual(pairView({ pairId: 'pair-1', paused: false, signedOut: false, status: st, minuteOfDay: NOON }), {
    kind: 'error', message: 'pair-1: list docs://a: HTTP 502',
  });
  assert.deepEqual(pairView({ pairId: 'pair-2', paused: false, signedOut: false, status: st, minuteOfDay: NOON }), { kind: 'watching' });
  const proc = running({ running: false, errors: { '*': 'sync stopped unexpectedly (exit 1)' }, lastError: 'sync stopped unexpectedly (exit 1)' });
  assert.deepEqual(pairView({ pairId: 'pair-2', paused: false, signedOut: false, status: proc, minuteOfDay: NOON }), {
...
test('the folder line: "waiting for the window" only while the clock is outside it', () => {
  ...
  assert.deepEqual(pairView({ pairId: 'pair-1', paused: false, signedOut: false, status: st, minuteOfDay: 23 * 60 }), { kind: 'watching' });
});
