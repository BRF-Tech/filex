...
// The executable the OS starts at sign-in: under the Windows PORTABLE build
// execPath is a fresh extraction folder on every launch, gone by the next
// sign-in — and with the preference following the OS, the switch turned itself
// off at every start. The portable .exe (and an AppImage) is what is run.
test('the login item runs the portable exe or the AppImage, not the extraction', () => {
  const tmp = 'C:/Users/a/AppData/Local/Temp/2abc/filex.exe';
