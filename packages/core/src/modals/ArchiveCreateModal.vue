...
const supportsEncryption = computed(() => format.value === 'zip' || format.value === '7z');

function formatLabel(value: ArchiveCreateFormat): string {
  return value === '7z' ? '7z' : value.toUpperCase();
}
      ...
      <template v-if="format === '7z'">
        ...
        <label class="fe-field">
          ...
          <select v-model.number="dictionarySizeMB" class="fe-input">
            <option v-for="size in [4, 8, 16, 32, 64, 128, 256]" :key="size" :value="size">{{ size }} MiB</option>
          </select>
