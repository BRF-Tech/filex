...
const archiveAllowedFormats = computed<ArchiveCreateFormat[]>(() => {
  ...
  const formats = configured.filter((format): format is ArchiveCreateFormat => ARCHIVE_CREATE_FORMATS.includes(format));
  return formats.length ? formats : ARCHIVE_CREATE_FORMATS;
});
const archiveDefaultFormat = computed<ArchiveCreateFormat>(() => {
  ...
  return archiveAllowedFormats.value.includes('7z') ? '7z' : archiveAllowedFormats.value[0];
});
...
function selectionActionList(sel: FileNode[]): ContextAction[] {
  ...
  return [
    ...
    { key: 'archive-create', label: t('ctx.archive_create'), hidden: !any || (single && isArchive) || !w || e2eActive.value, disabled: !any },
...
function archiveRequestError(err: unknown): string {
  ...
  if (detail) {
    try {
      ...
      if (body.code === 'PASSWORD_REQUIRED') return t('archive.password_required');
      if (body.code === 'BAD_PASSWORD') return t('archive.bad_password');
      if (body.code === 'TARGET_EXISTS') return t('archive.target_exists');
...
async function startArchiveExtraction(
  ...
  try {
    ...
    else archiveError.value = message;
  } finally {
    archiveBusy.value = false;
  }
}
